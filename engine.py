...
from lottie.objects.shapes import GradientFill, GradientType, RoundedCorners
from lottie.objects.effects import GaussianBlurEffect
from lottie.objects import easing
from lottie.objects.helpers import BlendMode
...
def led_region_at(path, x, y, radius=50, threshold=130):
    """Размер и цвет LED в окне вокруг координаты (для мягкого shape-слоя)."""
    import numpy as np
    from PIL import Image as PILImage
    ...
    for thr in (threshold, threshold - 20, threshold - 40, 90):
        ...
        bh = max(2, int(ys.max() - ys.min() + 1))
        # чуть расширяем, чтобы мягкий градиент накрывал индикатор
        rw = min(90, max(20, int(bw * 1.35)))
        rh = min(22, max(5, int(bh * 1.5)))
        cr = float(crop[local][:, 0].mean() / 255)
        cg = float(crop[local][:, 1].mean() / 255)
        cb = float(crop[local][:, 2].mean() / 255)
        return {"x": cx, "y": cy, "w": rw, "h": rh, "color": [cr, cg, cb]}

    return {"x": cx, "y": cy, "w": 40, "h": 8, "color": [0.06, 0.85, 0.98]}
...
class Ctx:
    ...
    def led_glow_layer(self, x, y, w, h, color, name="led", corner=None, blur=0):
        """Мягкий LED-слой: скруглённый rect + линейный градиент, blend Screen."""
        layer = ShapeLayer()
        layer.name = name
        layer.blend_mode = BlendMode.Screen

        if corner is None:
            corner = max(2, min(w, h) * 0.4)
        ...
        rc.position.value = [0, 0]
        grp.add_shape(rc)
        grp.add_shape(RoundedCorners(corner))

        c = parse_color(color)
        gf = GradientFill()
        gf.gradient_type = GradientType.Linear
        gf.start_point.value = [-w / 2, 0]
        gf.end_point.value = [w / 2, 0]
        gf.colors.set_stops([
            (0.0, Color(c.r, c.g, c.b, 0)),
            (0.28, Color(c.r, c.g, c.b, 0.45)),
            (0.5, Color(c.r, c.g, c.b, 0.85)),
            (0.72, Color(c.r, c.g, c.b, 0.45)),
            (1.0, Color(c.r, c.g, c.b, 0)),
        ])
        grp.add_shape(gf)
        layer.add_shape(grp)
        layer.transform.position.value = [x + self.ox, y + self.oy]
        if blur > 0:
            blur_fx = GaussianBlurEffect()
            blur_fx.effects[0].value.value = blur
            layer.effects = [blur_fx]
        layer.in_point, layer.out_point = 0, self.frames
        return layer
